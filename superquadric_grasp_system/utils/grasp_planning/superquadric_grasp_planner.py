...
def principal_axis_sweeps(S: Superquadric, G: Gripper, step_deg=10):
    ...
    print(f"[INFO] principal axis sweep complete: {len(candidate_R)} candidates (expected: {expected_total})")
...
# ------------------------------------------------------------
# SuperquadricGraspPlanner class for ROS2 integration
# ------------------------------------------------------------
...
class SuperquadricGraspPlanner:
    ...
    def plan_grasps(self, point_cloud_path, shape, scale, euler, translation, max_grasps=5, visualize_support_test=False, visualize_collision_test=False):
        """
        ...
        try:
            ...
            for Rg, tg in G_raw:
                if support_test(Rg, tg, S, G, kdtree, debug_mode=visualize_support_test, max_debug_calls=3):  # Show first 3
            ...
            for Rg, tg in G_after_support:
                if collision_test(Rg, tg, S, G, kdtree, debug_mode=visualize_collision_test, max_debug_calls=3):
            ...
            for i, (score, (Rg, tg)) in enumerate(scored_grasps):
                ...
                print(f"Diverse Grasp {i+1}: score={score:.4f}, pos=[{tg[0]:.3f}, {tg[1]:.3f}, {tg[2]:.3f}], approach_dir=[{approach_vector[0]:.3f}, {approach_vector[1]:.3f}, {approach_vector[2]:.3f}]")
    ...
    def select_best_grasp_with_criteria(self, grasp_data_list, object_center=None, point_cloud=None):
        ...
        try:
            if not grasp_data_list:
                print("[WARNING] No grasp data provided")
                return None
                ...
                try:
                    ...
                    if point_cloud is not None:
                        try:
                            ...
                            print(f"    [WARNING] Point cloud distance calculation failed: {pc_error}")
                    ...
                    if original_idx < 3:
                        print(f"  [SCORE] Grasp {original_idx+1}:")
                        print(f"    Position: [{pos_x:.3f}, {pos_y:.3f}, {pos_z:.3f}]")
                        print(f"    Approach Z: {approach_z:.3f}")
                        print(f"    Distance to base: {distance_to_base:.3f}m")
                    ...
                    print(f"Error scoring grasp {original_idx+1}: {e}")
                    continue
            
            if best_grasp is not None:
                print(f"\n[SELECTION] Best grasp selected (#{best_info['original_index']+1}) with score: {best_score:.6f}")
                print(f"   Base score: {best_info['base_score']:.6f}")
                print(f"   Distance score: {best_info['distance_score']:.3f}")
                print(f"   Height score: {best_info['height_score']:.3f}")
                print(f"   Approach score: {best_info['approach_score']:.3f}")
                
                if best_info.get('min_distance_to_cloud') is not None:
                    print(f"   Min distance to point cloud: {best_info['min_distance_to_cloud']*1000:.1f}mm")
            ...
            print(f"Error in select_best_grasp_with_criteria: {e}")
            import traceback
            traceback.print_exc()
            return None
